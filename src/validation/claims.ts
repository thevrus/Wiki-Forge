...
export function extractClaims(markdown: string): string[] {
  ...
  // Remove code blocks and markdown tables — tables commonly contain
  // package names and library names that aren't source-code identifiers
  const withoutCodeBlocks = body.replace(/```[\s\S]*?```/g, "")
  const withoutTables = withoutCodeBlocks.replace(/^\|.*\|$/gm, "")

  // Match inline backtick content
  const matches = withoutTables.match(/`([^`]+)`/g) ?? []
...
export function verifyClaims(
  ...
  for (const claim of claims) {
    ...
    if (sourceContent.includes(claim)) {
      ...
      const found =
        sourceContent.includes(claim.split("/").pop()!) || // try filename only for paths
