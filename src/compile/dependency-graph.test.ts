...
test("extracts ES import dependencies", () => {
  const files = [
    ...
    {
      path: "src/handler.ts",
      content: `import { validate } from './utils/validate'`,
    },
    {
      path: "src/db.ts",
...
test("extracts dynamic import() dependencies", () => {
  const files = [
    {
      path: "src/cli.ts",
      content: `const mod = await import('./commands/init')`,
    },
    {
      path: "src/commands/init.ts",
      content: `export async function runInit() {}`,
    },
  ]
...
test("resolves index files", () => {
  const files = [
    {
      path: "src/app.ts",
      content: `import { createProviders } from './providers'`,
    },
    {
      path: "src/providers/index.ts",
      content: `export function createProviders() {}`,
    },
  ]
