...
function resolveRelative(specifier: string, importerPath: string): string {
  const dir = dirname(importerPath)
  return normalize(join(dir, specifier)).replace(/\\/g, "/") // normalize Windows paths
}

/** Find the best matching file in the file set for a resolved import path. */
function findMatch(resolved: string, fileSet: Set<string>): string | undefined {
  // Exact match
