...
import { extractClaims, verifyClaims, verifyDocClaims } from "./verify-claims"
...
`
  ...
  test("verifies claims that exist in source", () => {
    const result = verifyClaims(["handleBooking", "SLOT_DURATION_MIN", "AuthService"], source)
  ...
  test("identifies unverified claims", () => {
    const result = verifyClaims(["handleBooking", "NonExistentFunction", "FAKE_CONSTANT"], source)
