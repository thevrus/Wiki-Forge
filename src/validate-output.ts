...
export function validateCompiledOutput(raw: string): ValidationResult {
  ...
  if (placeholderCount > 0) {
    warnings.push(
      `${placeholderCount} section(s) contain placeholder text`,
    )
  }

  // Warn if no section has real depth
  if (richCount === 0) {
    warnings.push("No section has substantial content (80+ chars excluding metadata)")
  }
  ...
  for (const block of mermaidBlocks) {
    ...
    const validStarts = [
      ...
      "classDiagram", "erDiagram", "gantt", "pie", "mindmap", "timeline",
    ]
